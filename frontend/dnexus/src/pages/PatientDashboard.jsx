...
function PatientDashboard() {  
  ...
  useEffect(() => {
    if (actorIdState) {
      setActorIdState(actorIdState);
      localStorage.setItem('actorId', actorIdState);
    }
  }, [actorIdState]);
...
//           setconsultations(response.data);
  ...
  const handleWithdrawConsent = async (consentId) => {
    try {
      const response = await axios.put(`http://localhost:8085/core/consent/${consentId}/status?newStatus=WITHDRAWN`);
      console.log("Withdraw Consent Response:", response.data);

      setShowModal(false);

      // Assuming successful withdrawal, you may want to update the UI accordingly

    } catch (error) {
      console.error('Error withdrawing consent:', error);
      setError('Error withdrawing consent. Please try again.');
    }
  };
            ...
            <div className="consent-modal-content">
              ...
              {selectedConsent && selectedConsent.map((consent) => (
                  <tr key={consent.id}>
                      ...
                      <td>{consent.consentStatus}</td>
                      <td>
                          {consent.consentStatus !== 'NONE' && consent.consentStatus !== 'WITHDRAWN' && (
                              <Link className='consent-withdraw-link' onClick={() => handleWithdrawConsent(consent.id)}>Withdraw</Link>
                          )}
                      </td>
                  </tr>
              ))}
